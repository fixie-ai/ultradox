...
| playDtmfSounds | Plays dual-tone multi-frequency (a.k.a dialpad) tones. See [DTMF](/telephony/overview#dtmf) for details on sending and receiving tones in your voice applications. |
...
You give your AI agent access tools when you [create the call](/api-reference/calls/calls-post) or create a new [call stage](/agents/call-stages#creating-and-managing-stages).
...
<Accordion title="Client tools require webrtc or websocket call medium.">
  ...
  You can also use client tools with a websocket medium. See the `ClientToolInvocation` and `ClientToolResult` [data messages](/protocol/datamessages).
...
For tools that take even longer, consider responding immediately and later using an [input_text_message](/protocol/datamessages#inputtextmessage)
