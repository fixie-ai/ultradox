...
<Danger>
**Calls May Fail With Unsupported Models:**
Not setting a model string or using any legacy model strings other than one that is [currently supported](#available-models-and-status) may cause your calls to fail. Move to a supported string now.
</Danger>
...
* **Call Failures** → Agents and calls using an empty model string or the default `fixie-ai/ultravox` model will fail.
* Agents and calls using `ultravox-v0.6` (or `ultravox-v0.6-llama3.3-70b`)  will continue to use Llama 3.3 70B.
...
|----------------------|----------------------------------------------------------------------|--------|
| Ultravox Default     | `fixie-ai/ultravox`                                                  | Currently points to Llama 3.3 70B<br/>Will cause calls to fail beginning Dec 22 |
