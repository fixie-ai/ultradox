...
* **More Improvements** → Added [`vadSettings`](/api-reference/calls/calls-post) parameters for more control, new public [`hang-up`](/essentials/tools#changing-call-state) tool, and added [`timeout`](/api-reference/schema/base-tool-definition) for tools.
