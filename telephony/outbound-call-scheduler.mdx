...
<Danger>
  ...
  <br />
  Unless your call batch is scheduled without and end date/time, using OCS does not guarantee that all calls will be made in the window you specify.
</Danger>
...
<ParamField body="calls" type="array" required>
  ...
  - `medium` - Call medium (can be `sip`, `twilio`, `telnyx`, or `plivo`)
...
The OCS supports three execution methods:

### Method 1: Automatic SIP Execution
When all calls in your batch uses the `sip` medium, you can provide `outgoing` details and Ultravox will initiate calls automatically:

```js Automatic SIP Execution Example
{
  "calls": [
    {
      "medium": {
...
### Method 2: Built-in Telephony
When you have configured Twilio, Telnyx, or Plivo credentials, you can provide `outgoing` details and Ultravox will initiate calls automatically using your provider:
...
When you need something custom, Ultravox notifies your webhook when capacity is available so that you can join the call:
