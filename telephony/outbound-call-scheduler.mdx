...
The Outbound Call Scheduler (OCS) enables you to create and manage batches of outbound calls without worrying about concurrency limits or retry logic. Simply define a time window and upload your call list - Ultravox handles the rest.
...
- **SIP Integration** - Use the `sip` call medium for 'set it and forget it' simplicity
- **Webhook Integration** - Get notified when calls are ready to be initiated for custom connectivity
...
<Steps>
  ...
  <Step title="Automatic Processing">
    Ultravox processes calls within your specified window, respecting concurrency limits.
  </Step>
  <Step title="Call Execution">
    Calls are either initiated automatically (SIP) or via webhook notifications.
  </Step>
