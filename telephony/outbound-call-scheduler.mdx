...
<Tabs>
  ...
  <Tab title="Example: Automatic Execution" icon="bolt-auto">
    When all calls in your batch have an `outgoing` `sip` medium, or if you are using Twilio, Telnyx, or Plivo (and you have configured credentials), Ultravox initiates calls automatically:

    ```js Example: Automatic Execution (SIP)
    {
      "calls": [
        {
          "medium": {
    ...
    ```js Example: Automatic Execution (Telephony)
    {
      "calls": [
        {
          "medium": {
  ...
  <Tab title="Example: Webhook Notification" icon="webhook">
    ...
    Your webhook receives a [Call Definition](/api-reference/schema/call-definition) payload:
