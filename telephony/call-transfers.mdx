...
<Note>
  <b>SIP Bridge Transfers Cost More</b>

  Using bridge transfers with the `sip` medium will result in SIP charges beyond the Ultravox call duration as our SIP infrastructure remains active to bridge audio between the caller and human operator. See [our SIP page](/telephony/sip#sip-billing) for details.
</Note>
...
<Info>
  ...
  <br />
  The built-in `coldTransfer` tool also works with Twilio if you've [added your credentials](/telephony/telephony-platforms#providing-telephony-credentials), but be aware that invoking the tool will immediately end the Ultravox call regardless of whether the transfer succeeds.
</Info>
