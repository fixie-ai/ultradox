...
If no regexes match, the call will be sent to your [fallback hanlder](#fallback-handler) if configured and rejected otherwise.
...
If your handler times out, returns a non-2XX status, or returns a malformed/invalid response, Ultravox will reject the SIP call (with code `504` for timeouts and `502` otherwise).
...
You can use SIP headers to populate template context for incoming SIP calls. Each header value is interpretted as JSON to allow for complex values.
