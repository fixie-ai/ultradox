...
Note that there is also a platform-enforced concurrency limit. A call can create as many threads as are useful, but the number
...
its generation request will be enqueued and will run once another thread finishes generating.
...
When a thread receives a message, it gets added to a queue. If the thread was IDLE, it will transition to handle the message immediately. Otherwise it will
...
If you listen for generations, it is highly recommended to use your own thread IDs when spawning threads so you can correlate these messages with their thread.
...
Suppose you want some other LLM-based observer to periodically check how a call is going and then call a tool to report progress or effect changes.
...
responding with a `send-to-thread` response type and a `SpawnThread` message that uses "UI" as the `parentThreadId`, sets its own threadId as the
...
```js
{
  ...
  "result": JSON.stringify({
    ...
    "dataMessage": {
      ...
      "additionalMessages": [
        // If there are messages you want to retain from the thread's prior history, include them here.
