...
### Methods
The `UltravoxSession` contains methods for joining/leaving a call, for sending text messages to the model, for registering client tools, and for muting the microphone/speaker.
...
|----------------- | ------------------------------------------- |
| name | String. The name of the tool. Must match what is defined in `selectedTools` during [call creation](../api/calls/#create-call). |
...
**Accepts `parameters`** → An object containing key-value pairs for the tool's parameters.
...
**`implementationMap`** → An object where each key (a string) represents the name of the tool and each key's value is a `ClientToolImplementation` function.
...
Client tools are defined just like "server" tools with a couple of exceptions:
