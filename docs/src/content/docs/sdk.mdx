...
When the tool call completes, the message contains an array of messages. The first is the tool call. The second is the tool result. These messages are also available via [List Call Messages](../api/calls/#list-call-messages)
