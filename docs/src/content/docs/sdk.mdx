...
The Ultravox [REST API](./api/auth/) is used to create calls but you must use one of the Ultravox client SDKs to join and end calls. This page uses examples in JavaScript. The concepts are the same across all the [SDK flavors](#sdk-flavors).

## Ultravox Session
The core of the SDK us the `UltravoxSession`. The session is used to join and leave calls.
