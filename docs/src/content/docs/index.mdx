...
## `ultravox-client`
You will need to use the `ultravox-client` in any web-based applications. The REST API is used to create calls but the client is needed to join (and end) calls. You can [find it](https://www.npmjs.com/package/ultravox-client) in the npm registry. Please let us know if you need a client for a specific language or framework.
