    ...
    <table class="w-full">
      ...
      <tr>
       ...
       <tr>
        ...
        <td>string</td>
        <td>The initial medium (MESSAGE_MEDIUM_VOICE or MESSAGE_MEDIUM_TEXT) to use for the call. Defaults to MESSAGE_MEDIUM_VOICE. Can be changed by using [`setOutputMedium`](../../sdk#setoutputmedium) from the Ultravox client SDK.</td>
      </tr>
      <tr>
        <td class="font-mono">languageHint</td>
        <td>string</td>
      ...
      <tr>
        ...
        <td>boolean</td>
        <td>Defaults to `true`. Setting to `false` (not recommended) enables live, user transcripts which may not match what the model hears. Instead, we recommend using [List Call Message](#list-call-messages) to get accurate user transcripts after the call completes. </td>
      </tr>
      <tr>
        <td class="font-mono">voice</td>
        <td>string</td>
