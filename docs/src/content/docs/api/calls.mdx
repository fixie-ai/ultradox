...
### inactivityMessages
If the caller becomes silent or stops interacting, the agent will use these messages to see if the caller is still there. If the caller ultimately never responds, the agent can hang up the call. The agent will use the messages in order and are delivered by the agent after the specified duration elapses.

Messages are delivered after the user fails to interact for the specified duration. Messages are delivered in order. If the user interacts at any point during inactivity messages being delivered by the agent, the first message will be delivered again after the next period of user inactivity.
  ...
  <div>
    ...
    <dd class="ml-8">
      <ul class="list-disc space-y-2">
        ...
        <li><code>END_BEHAVIOR_HANG_UP_STRICT</code> - Immediately terminates the call after speaking the message, without waiting for any additional user interaction.</li>
      </ul>
    </dd>
  </div>
</dl>
...
<Steps>
1. Call starts.
1. After 5 seconds of no user interaction, agent says "Hello, are you still there?".
1. If user interacts, call continues. If no user interaction occurs for 10 seconds, agent says "If there's nothing else, may I end the call?".
1. If no user interaction occurs for another 5 seconds, agent says the provided message and the call ends.
</Steps>
...
Mistral uses a simplified prompt structure.
:::
