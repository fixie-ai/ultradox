...
### Join the Call
Now that we have a `joinUrl`, we can use the `ultravox-client` in our application to join the call. The `ultravox-client` can be [found in npm](https://www.npmjs.com/package/ultravox-client).
