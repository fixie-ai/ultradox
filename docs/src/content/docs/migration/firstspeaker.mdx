...
`firstSpeaker` is more clear an indicates who speaks first during the call. For in-bound calls, the default (`FIRST_SPEAKER_AGENT`) will have the agent answer the call. For outgoing calls, setting it to `FIRST_SPEAKER_USER` indicates the expecation that most people answer a phone call with "Hello?".
