...
```js
{
  ...
  "knownToolResults": [  // Optional, skips execution of matching tool calls using these results intead
    {
      "invocationId": "unique-invocation-id",
      "result": "Tool execution result",
      "responseType": "tool-response",
...
<ResponseField name="newThreadId" type="string">
  Unique identifier for the new thread. If omitted, an id will be auto-generated. If present and a thread with the same ID already exists, this spawn attempt will be rejected.
</ResponseField>
<ResponseField name="parentThreadId" type="string" default="UI">
  The thread to fork from. Defaults to "UI" (the main conversation).
</ResponseField>
...
<ResponseField name="limits" type="object">
  ...
  * `generationInputTokenLimit` (integer) → Approximate (uncached) input token limit per generation.
