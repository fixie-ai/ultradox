...
| Threads      | [SideGenerationFailed](#sidegenerationfailed)                                                                   | A thread's generation has failed.                            |
...
<ResponseField name="newThreadId" type="string">
  Unique identifier for the new thread. If omitted, a UUID is auto-generated.
</ResponseField>
<ResponseField name="parentThreadId" type="string" default="UI">
  The thread to fork from. Defaults to "UI" (the main conversation). Set this to an existing thread ID to create a nested thread.
</ResponseField>
<ResponseField name="additionalMessages" type="array">
  Messages to append to the forked history before the thread starts generating. Each element must be either a [UserTextMessage](#usertextmessage) or [ForcedAgentMessage](#forcedagentmessage) (with an added `type` field).
</ResponseField>
<ResponseField name="toolFilter" type="object">
  Restricts which tools the thread can use. Specify either `allowedTools` (allowlist) or `disallowedTools` (blocklist), but not both.
</ResponseField>
<ResponseField name="limits" type="object">
  ...
  * `generationInputTokenLimit` (integer) → Max input tokens per generation.
  * `threadFuzzyInputTokenLimit` (integer) → Approximate total input token limit.
</ResponseField>
...
<ResponseField name="reason" type="string" required>
  Why the thread was terminated. Common reasons include: limit reached, inference error, or cancellation.
</ResponseField>

### SideGenerationDelta
Streaming text delta from a thread's LLM generation, similar to a transcript delta.
...
Sent when a thread's generation round finishes successfully.
...
### SideGenerationFailed
Sent when a thread's generation round fails.

**Message Structure**
```js
{
  "type": "side_generation_failed",
  "threadId": "my-thread-1"
}
```
...
**Fields**
<ResponseField name="threadId" type="string" required>
  The ID of the thread whose generation failed.
</ResponseField>
