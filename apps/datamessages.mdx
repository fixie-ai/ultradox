...
Data messages are used to communicate non-audio information between your client and the Ultravox service during calls. These messages enable real-time control and interaction with ongoing conversations beyond just audio streams.

## Communication Methods
* **WebRTC Data Channels** → Used by our [SDKs](/apps/sdks) and [WebSocket](/apps/websockets) connections for bi-directional, real-time message exchange during calls. This is the primary method for client apps to interact with calls.
...
| Type         | Message                                       | Description                                            |
| ------------ | --------------------------------------------- | ------------------------------------------------------ |
| Conversation | [CallStarted](#callstarted)                   | Notifies that a call has started.                      |
...
<ResponseField name="state" type="string" required>
  Current sesssion state. Common values include "listening", "thinking", and "speaking".
</ResponseField>
...
<ResponseField name="urgency" type="string" default="soon">
  ...
  * soon → Doesn't interrupt but starts generation at the next opportunity.
  * later → Message is considered during the next natural generation without forcing new generation.
</ResponseField>
...
<ResponseField name="invocationId" type="string" required>
  Unique identifier for this invocation. Must be included in the corresponding ClientToolResult.
</ResponseField>
...
<Note>
<b>Data Connection Equivalent: DataConnectionToolInvocation</b>
<br />
If you are using a Data Connection, you need to use `DataConnectionToolInvocation` instead.
</Note>
...
**Fields**
<ResponseField name="invocationId" type="string" required>
  Must match the invocationId from the corresponding ClientToolInvocation.
</ResponseField>
<ResponseField name="result" type="string">
  Tool execution result, often a JSON string. May be omitted for errors.
</ResponseField>
...
<ResponseField name="errorMessage" type="string">
  Human-readable error description if the tool failed.
</ResponseField>
...
```
<Note>
  This message is only available on WebSocket connections and is critical for proper interruption handling.
</Note>
...
<ResponseField name="uninterruptible" type="boolean">
  If true, prevents interruption while speaking
</ResponseField>
<ResponseField name="urgency" type="string" default="soon">
  Controls when the message is processed. Must be either "immediate" (process immediately) or "soon" (process at next opportunity).
</ResponseField>
