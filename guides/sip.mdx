...
### Incoming SIP Calls
Incoming calls are initiated by a SIP client to your Ultravox agent. When the Ultravox incoming SIP call is created, we return a join URL that is then used to connect to the agent.
    ...
    medium: {
      sip: {
        outgoing: {
          ...
          from: "<company_name>",
          username: "<linphone_username>",
          password: "<linphone_password>"
        }
      }
    }
    ```
