---
title: 'External TTS'
description: 'Use Ultravox with your own text-to-speech provider'
icon: 'volume-2'
---
...
there are a few different providers available. The named providers (such as ElevenLabs and Cartesia) are ones
with whom we've specifically integrated to make their voices work smoothly with Ultravox. This typically means
we've taken the time to integrate with their streaming API and take advantage of audio timing information from
the provider to synchronize transcripts.  While you'll still need to work with the provider to ensure your
agent's requests will be fulfilled reliably and quickly, you can be confident that requests will be valid and
responses will be used sensibly.
...
PlayHT's latest dialog model provides high quality voices, albeit with a bit higher latency. Unfortunately their
API doesn't allow for input streaming and they don't provide audio timing information. The lack of input streaming
forces Ultravox to buffer text before sending it to PlayHT which can increase agent response latency a bit. It
also makes prosody issues at sentence boundaries more likely, though we haven't seen this often in practice.

With PlayHT, transcript timing must be approximated. Once we have a full generation (and therefore the true audio
duration), Ultravox assumes each character requires the same duration and approximates transcripts based on that.
While the first generation is still streaming, we rely on an estimated words-per-minute speaking rate we've determined
across many PlayHT generations.
...
in their streaming API yet and Ultravox has no battle-tested integration with it yet as a result. That said, you can use
our "generic" ExternalVoice to give Deepgram a try now using their REST API.
