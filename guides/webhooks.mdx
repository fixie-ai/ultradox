...
The `{event_name}` is the same as the event you subscribed to when creating your webhook. The `{call_object}` matches what is returned from the [Get Call](/api-reference/calls/calls-get) endpoint.
