...
Language models like Llama 3.3 70B (which powers Ultravox) are inherently stochastic systems. Their power comes from their flexibility - the ability to handle a wide range of inputs and generate contextually appropriate, creative responses. However, this same flexibility makes them challenging to control in scenarios where you need predictable, repeatable behavior.
...
- **Cognitive overload**: Long prompts can cause the model to forget or confuse instructions
- **Lack of focus**: The agent must juggle all possible paths simultaneously
...
#### Real-World Example
**Language Learning Assistant (TODO: do we include this as example prompt/code?)**

A language learning assistant that helps users practice conversations in different languages. The system prompt defines the assistant's supporting role, specifies how to handle corrections, provides examples of appropriately challenging vocabulary, and outlines conversation scenarios at different proficiency levels. All of this fits within a single comprehensive prompt because the task, while requiring nuance, follows consistent patterns regardless of language or topic.

### Approach 2: Inline Instructions
...
- **Call Tool State**: Pass context between tool calls to maintain state
...
4. Call Tool State maintains context between turns
...
#### Call Tool State
Call Tool State allows you to maintain state between tool calls, passing context from one call to the next. This is particularly useful for guiding the agent through a multi-step process.

TODO link to api docs/places where this is set/retrieved
TODO code snippet/example

The agent will not see the Call Tool State directly. It allows you to pass information between tool calls and then use that information inside tools and to impact the responses from tool calls.

#### Tool Response Messages
Instead of having a tool call result send a 200 with "Successfully entered customer information", you can provide an instruction of what the agent should do next (e.g. "Successfully entered customer information. Now you MUST lookup the answer to their question").
...
const schedulingTool = {
  ...
  parameters: {
    ...
    required: ["action"],
    // Automatically include the current Call Tool State
...
By leveraging building blocks like deferred messages, Call Tool State, and targeted tool response messages, you can create sophisticated conversational flows that guide users through complex processes while maintaining the natural feel of human conversation.