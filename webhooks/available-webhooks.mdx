...
The call's messages are immutable because the call is over. However, billing information may not be available yet (in particular for SIP calls, where the SIP session could be ongoing...see [call.billed](#call-billed). This event is also sent for unjoined calls when their join timeout is reached.
